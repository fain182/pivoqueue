...
describe StoriesPage do
  context "when apikey is valid" do
    before(:each) do
      @user = mock(Object, :logged? => true, :apikey=>IO.read('.token'))
      @stories_page = StoriesPage.new @user
    end
    it "shows the iteration in html" do
      iteration = Iteration.new(@user)
      @stories_page.should be_include iteration.to_html
    end
  end
